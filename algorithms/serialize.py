...
def serialize(schema: List[Union[Tuple[str], Tuple[str, str]]], rows: List[Any]) -> bytes:
    ...
    for row in rows:
        ...
        new_row = []
        length_bytes = CELL_SIZE_BY_MARK[CellType.UNSIGNED_INT[0]]
        for i, cell in enumerate(row):
            length_bytes += CELL_SIZE_BY_MARK[schema[i][0]]
            if len(schema[i]) == 2:
        ...
        result.append(struct.pack(f'={CellType.UNSIGNED_INT[0]}{pattern}', length_bytes, *new_row))

    return b''.join(result)


def deserialize(schema: List[Union[Tuple[str], Tuple[str, str]]], block: bytes) -> Tuple[List[Any], bytes]:
    ...
    row_length_size = CELL_SIZE_BY_MARK[CellType.UNSIGNED_INT[0]]
    byte_index = 0
    row_size = struct.unpack('=' + CellType.UNSIGNED_INT[0], block[:row_length_size])[0]
    while block_size - byte_index >= row_size:
        ...
        while row_size > 0:
            ...
            if pattern.startswith('{'):
                ...
                value_size = CELL_SIZE_BY_MARK[pattern]
        ...
        if block_size - byte_index >= row_length_size:
            row_size = struct.unpack('=' + CellType.UNSIGNED_INT[0], block[byte_index:byte_index + row_length_size])[0]
        else:
            break
